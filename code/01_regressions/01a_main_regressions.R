...
# after 
reg0 <- felm(data = dataAfter, co2Mass ~ btc + costMWh | factor(year) + factor(month) + factor(dow)|0|0)

reg1 <- felm(data = dataAfter, co2Mass ~ btc + costMWh + paLMP| factor(year) + factor(month) + factor(dow)|0|0)
reg2 <- felm(data = dataAfter, co2Mass ~ btc + costMWh + mWhPerBTC | factor(year) + factor(month) + factor(dow)|0|0)

reg3 <- felm(data = dataAfter, co2Mass ~ btc + costMWh + paLMP | factor(year) + factor(month) + factor(dow)+ factor(btcDiffEra)|0|0)
reg4 <- felm(data = dataAfter, co2Mass ~ btc + costMWh + mWhPerBTC  | factor(year) + factor(month) + factor(dow) + factor(btcDiffEra)|0|0)

# MAIN TABLE 1 - PANEL A 
stargazer(reg1, reg2, reg3, reg4,
          ...
          out="output/01_regressions/01_main_reg_panelA.tex")

# before 
reg0 <- felm(data = dataBefore, co2Mass ~ btc + costMWh | factor(year) + factor(month) + factor(dow)|0|0)
reg1 <- felm(data = dataBefore, co2Mass ~ btc + costMWh + paLMP | factor(year) + factor(month) + factor(dow)|0|0)
reg2 <- felm(data = dataBefore, co2Mass ~ btc + costMWh + mWhPerBTC | factor(year) + factor(month) + factor(dow)|0|0)

reg3 <- felm(data = dataBefore, co2Mass ~ btc + costMWh + paLMP | factor(year) + factor(month) + factor(dow)+ factor(btcDiffEra)|0|0)
reg4 <- felm(data = dataBefore, co2Mass ~ btc + costMWh + mWhPerBTC  | factor(year) + factor(month) + factor(dow) + factor(btcDiffEra)|0|0)

# MAIN TABLE 1 - PANEL B
stargazer(reg1, reg2, reg3, reg4,
          type="text", df=F,
          report=("vc*sp"), 
          column.sep.width = c("10pt"),
          ...
          add.lines = list(c("$C_{gen}$ Control", "Y", "Y", "Y", "Y"),
                           c("$P_{elec}$ Control", "Y", "-", "Y", "-"),
                           c("$E_{BTC}$ Control", "-", "Y", "-", "Y"),
                           c("Mean BTC", round(meanBTCBefore,0), round(meanBTCBefore,0), round(meanBTCBefore,0), round(meanBTCBefore,0)),
                           c("Mean CO2", round(meanCO2Before,0), round(meanCO2Before,0), round(meanCO2Before,0), round(meanCO2Before,0)),
                           c("Elasticity", round(summary(reg1)$coefficients[1] * (meanBTCBefore/meanCO2Before), 2), round(summary(reg2)$coefficients[1] * (meanBTCBefore/meanCO2Before), 2),  round(summary(reg3)$coefficients[1] * (meanBTCBefore/meanCO2Before), 2),  round(summary(reg4)$coefficients[1] * (meanBTCBefore/meanCO2Before), 2))),
          out="output/01_regressions/01_main_reg_panelB.tex")
